...
pub fn run_sparql_syntax_tests() -> ComplianceReport {
    ...
    let positive_tests = vec![
        ...
        ("syntax-basic-04", "PREFIX : <http://example.org/> SELECT * { :s :p :o }"),
        ("syntax-basic-05", "BASE <http://example.org/> SELECT * { <s> <p> <o> }"),
        
        // Projections
        ("syntax-select-01", "SELECT ?x WHERE { ?x ?p ?o }"),
        ("syntax-select-02", "SELECT ?x ?y WHERE { ?x ?p ?y }"),
        ("syntax-select-distinct", "SELECT DISTINCT ?x WHERE { ?x ?p ?o }"),
        ("syntax-select-reduced", "SELECT REDUCED ?x WHERE { ?x ?p ?o }"),
        
        // Query forms
        ("syntax-construct-01", "CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }"),
        ("syntax-construct-02", "CONSTRUCT { <http://example.org/s> <http://example.org/p> ?o } WHERE { ?s ?p ?o }"),
        ("syntax-ask-01", "ASK { ?s ?p ?o }"),
        ("syntax-describe-01", "DESCRIBE ?x WHERE { ?x ?p ?o }"),
        ("syntax-describe-02", "DESCRIBE <http://example.org/x>"),
        
        // Filters
        ("syntax-filter-01", "SELECT * WHERE { ?s ?p ?o FILTER (?o = 42) }"),
        ("syntax-filter-02", "SELECT * WHERE { ?s ?p ?o FILTER regex(?o, \"test\") }"),
        ("syntax-filter-03", "SELECT * WHERE { ?s ?p ?o FILTER (bound(?o)) }"),
        ("syntax-filter-04", "SELECT * WHERE { ?s ?p ?o FILTER (?o > 10 && ?o < 100) }"),
        ("syntax-filter-05", "SELECT * WHERE { ?s ?p ?o FILTER (?o > 10 || ?o < 0) }"),
        ("syntax-filter-06", "SELECT * WHERE { ?s ?p ?o FILTER (!bound(?o)) }"),
        
        // Literals
        ("syntax-lit-01", "SELECT * { ?s ?p \"string\" }"),
        ("syntax-lit-02", "SELECT * { ?s ?p \"string\"@en }"),
        ("syntax-lit-03", "SELECT * { ?s ?p \"42\"^^<http://www.w3.org/2001/XMLSchema#integer> }"),
        ...
        // OPTIONAL
        ("syntax-optional-01", "SELECT * WHERE { ?s ?p ?o OPTIONAL { ?s ?p2 ?o2 } }"),
        ("syntax-optional-02", "SELECT * WHERE { ?s ?p ?o OPTIONAL { ?s ?p2 ?o2 OPTIONAL { ?s ?p3 ?o3 } } }"),
        
        // UNION
        ("syntax-union-01", "SELECT * WHERE { { ?s ?p ?o } UNION { ?s ?p2 ?o2 } }"),
        ("syntax-union-02", "SELECT * WHERE { { ?s ?p ?o } UNION { ?s ?p2 ?o2 } UNION { ?s ?p3 ?o3 } }"),
        
        // Named graphs
        ("syntax-graph-01", "SELECT * WHERE { GRAPH <http://example.org/g> { ?s ?p ?o } }"),
        ("syntax-graph-02", "SELECT * WHERE { GRAPH ?g { ?s ?p ?o } }"),
        ...
        ("syntax-order-02", "SELECT * WHERE { ?s ?p ?o } ORDER BY ASC(?o)"),
        ("syntax-order-03", "SELECT * WHERE { ?s ?p ?o } ORDER BY DESC(?o)"),
        ("syntax-limit-01", "SELECT * WHERE { ?s ?p ?o } LIMIT 10"),
        ("syntax-offset-01", "SELECT * WHERE { ?s ?p ?o } OFFSET 5"),
        ("syntax-limit-offset", "SELECT * WHERE { ?s ?p ?o } LIMIT 10 OFFSET 5"),
        
        // Aggregates
        ("syntax-aggregate-01", "SELECT (COUNT(*) AS ?c) WHERE { ?s ?p ?o }"),
        ("syntax-aggregate-02", "SELECT (SUM(?o) AS ?sum) WHERE { ?s ?p ?o }"),
        ("syntax-aggregate-03", "SELECT (AVG(?o) AS ?avg) WHERE { ?s ?p ?o }"),
        ("syntax-aggregate-04", "SELECT (MIN(?o) AS ?min) WHERE { ?s ?p ?o }"),
        ("syntax-aggregate-05", "SELECT (MAX(?o) AS ?max) WHERE { ?s ?p ?o }"),
        ("syntax-aggregate-06", "SELECT ?s (COUNT(?o) AS ?c) WHERE { ?s ?p ?o } GROUP BY ?s"),
        ("syntax-aggregate-07", "SELECT ?s (COUNT(?o) AS ?c) WHERE { ?s ?p ?o } GROUP BY ?s HAVING (COUNT(?o) > 5)"),
        
        // BIND and VALUES
        ("syntax-bind-01", "SELECT * WHERE { ?s ?p ?o BIND (?o * 2 AS ?double) }"),
        ("syntax-values-01", "SELECT * WHERE { VALUES ?x { 1 2 3 } ?s ?p ?x }"),
        ("syntax-values-02", "SELECT * WHERE { ?s ?p ?o } VALUES ?x { 1 2 }"),
        
        // Subqueries
        ("syntax-subquery-01", "SELECT * WHERE { { SELECT ?s WHERE { ?s ?p ?o } } }"),
        
        // MINUS
        ("syntax-minus-01", "SELECT * WHERE { ?s ?p ?o MINUS { ?s <http://example.org/exclude> ?o } }"),
        
        // NOT EXISTS / EXISTS
        ("syntax-exists-01", "SELECT * WHERE { ?s ?p ?o FILTER EXISTS { ?s <http://example.org/p2> ?o2 } }"),
        ("syntax-not-exists-01", "SELECT * WHERE { ?s ?p ?o FILTER NOT EXISTS { ?s <http://example.org/p2> ?o2 } }"),
        
        // Property paths
        ("syntax-path-01", "SELECT * WHERE { ?s <http://example.org/p>/<http://example.org/q> ?o }"),
        ("syntax-path-02", "SELECT * WHERE { ?s <http://example.org/p>* ?o }"),
        ("syntax-path-03", "SELECT * WHERE { ?s <http://example.org/p>+ ?o }"),
        ("syntax-path-04", "SELECT * WHERE { ?s <http://example.org/p>? ?o }"),
        ("syntax-path-05", "SELECT * WHERE { ?s ^<http://example.org/p> ?o }"),
        ("syntax-path-06", "SELECT * WHERE { ?s (<http://example.org/p>|<http://example.org/q>) ?o }"),
        
        // SERVICE (federated queries)
        ("syntax-service-01", "SELECT * WHERE { SERVICE <http://example.org/sparql> { ?s ?p ?o } }"),
        ("syntax-service-02", "SELECT * WHERE { SERVICE SILENT <http://example.org/sparql> { ?s ?p ?o } }"),
    ];
    ...
    report.add_gap(ComplianceGap {
        ...
        reason: "Update operations (INSERT, DELETE, LOAD, etc.) parsing not implemented".to_string(),
...
pub fn run_sparql_update_tests() -> ComplianceReport {
    ...
    for (op, _desc) in update_operations {
        report.add_skipped(
            format!("update-{}", op.to_lowercase().replace(' ', "-").replace('/', "-")),
